...
4. Reconcile the execution contracts with VELDRA runtime modes before wiring autonomous execution; the settings screen now exposes bounded registry status, but ActionRunner still uses its established direct WebContainer/Shell path.
5. Verify provider/model metadata from primary sources before promoting catalog entries.

## Product capabilities

- **Implemented foundations:** provider registry, bounded execution-provider status in Runtime Settings, capability-based routing contracts, bounded budgets, entitlement policies, Image Studio unavailable state, Android fallback runtime, Remote Runtime allowlisted commands, and VELDRA update manifest boundary.
