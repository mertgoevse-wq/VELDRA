...
/** Use a filesystem-safe timestamp for downloads on all supported platforms. */
...
export function EventLogsTab() {
  ...
  const exportAsJSON = () => {
    try {
      ...
      a.href = url;
      a.download = `veldra-event-logs-${getExportTimestamp()}.json`;
  ...
  const exportAsCSV = () => {
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `veldra-event-logs-${getExportTimestamp()}.csv`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
  ...
  const exportAsPDF = () => {
    try {
      ...
      // Add VELDRA product subtitle
      doc.setFontSize(12);
      doc.setFont('helvetica', 'normal');
      doc.text('VELDRA - AI Development Workbench', margin, 45);
      yPos = 70;
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        // Add footer text
        doc.text('Generated by VELDRA', margin, doc.internal.pageSize.getHeight() - 10);
      ...
      // Save the PDF
      doc.save(`veldra-event-logs-${getExportTimestamp()}.pdf`);
  ...
  const exportAsText = () => {
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `veldra-event-logs-${getExportTimestamp()}.txt`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
              ...
              {exportFormats.map((format) => (
                <button
                  ...
                  className={classNames(
                    ...
                    'border border-bolt-elements-borderColor',
                    'hover:bg-accent-50 dark:hover:bg-accent-500/20',
