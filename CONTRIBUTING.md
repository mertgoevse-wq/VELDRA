...
Welcome! This guide provides all the details you need to contribute effectively to the project. Thank you for helping us make **VELDRA** a better provider-agnostic AI development workbench. 💡
...
```bash
docker run -p 5173:5173 --env-file .env.local veldra:development
```
...
```bash
docker build . --target veldra-production
```
...
```bash
docker run -p 5173:5173 --env-file .env.local veldra:production
```
